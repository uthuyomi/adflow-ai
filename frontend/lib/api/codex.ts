...
export type CodexConfiguration = { real_execution_enabled: boolean; manual_execution_enabled: boolean; mock_execution_enabled: boolean };
...
export const executeCodexReal = (id: string) => request<{ task: CodexTaskPrompt; execution: CodexExecution }>(`/codex-tasks/${id}/execute/real`, { method: "POST", body: JSON.stringify({ idempotency_key: `real-${id}-${crypto.randomUUID()}` }) });
