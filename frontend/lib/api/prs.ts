...
export type GitHubConnection = { id: string; github_login: string; status: string; last_verified_at: string | null };
...
export type GitHubConfiguration = { oauth_enabled: boolean };
...
export const startGitHubOAuth = () => request<{ authorization_url: string }>("/integrations/github/oauth/start", { method: "POST", body: JSON.stringify({ return_path: "/settings" }) });
export const connectGitHubToken = (token: string) => request<GitHubConnection>("/integrations/github/connections", { method: "POST", body: JSON.stringify({ token }) });
